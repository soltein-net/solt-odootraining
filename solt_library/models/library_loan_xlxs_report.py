...
class ReportePersonalizadoXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, loans):
        ...
        headers = ['ID del Préstamo', 'Título del Libro', 'Usuario', 'Fecha de Préstamo', 'Fecha de Devolución']
        ...
        for row_num, loan in enumerate(loans, start=1):
            ...
            sheet.write(row_num, 1, loan.book_id.name or '')
            sheet.write(row_num, 2, loan.user_id.name or '')
            # Escribir fechas como texto si son nulas
            sheet.write(row_num, 3, loan.loan_date.strftime('%Y-%m-%d') if loan.loan_date else '')
            sheet.write(row_num, 4, loan.expected_return_date.strftime('%Y-%m-%d') if loan.expected_return_date else '')
